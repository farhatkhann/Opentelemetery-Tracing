...
# parser.add_argument("--architecture", default="monolith", choices=["monolith", "microservices"])
# parser.add_argument("--service", default="spring-petclinic", help="Service name registered in Jaeger")
...
"""
Runs every workflow's k6 script, back to back, for a single sampling level
(and optionally across multiple run numbers).

Before EVERY new test, the Jaeger container is restarted so that the
previous test's Jaeger state/exporting does not interfere with the next test.
...
This calls run_experiment.py once per (workflow, run) combination, in order.
"""
...
# Jaeger Docker container name
JAEGER_CONTAINER = "pet-clinic-monolithic"
...
WORKFLOWS = [
    {
        "name": "browseOwners",
        "k6_script": "../k6/workflows/browseOwners.js"
    },
    {
        "name": "ownerRegistration",
        "k6_script": "../k6/workflows/ownerRegistration.js"
    },
    {
        "name": "ownerUpdate",
        "k6_script": "../k6/workflows/ownerUpdate.js"
    },
    {
        "name": "petRegistration",
        "k6_script": "../k6/workflows/petRegistration.js"
    },
    {
        "name": "visitRegistration",
        "k6_script": "../k6/workflows/visitRegistration.js"
    },
    {
        "name": "mixedWorkflow",
        "k6_script": "../k6/workflows/mixedWorkflow.js"
    },
]
...
parser.add_argument(
    "--sampling",
    required=True,
    help="Sampling configuration, e.g. sample10"
)
...
parser.add_argument(
    ...
    type=int,
    default=1,
    help="Number of runs per workflow (default 1)"
)
...
parser.add_argument(
    ...
    type=int,
    default=15,
    help="Seconds to wait between workflows"
)
...
parser.add_argument(
    ...
    action="store_true",
    help=(
        "Skip (workflow, run) combinations that already have "
        "a summary.json on disk."
    )
)
...
parser.add_argument(
    ...
    action="store_true",
    help=(
        "Abort the whole batch on the first run_experiment.py failure, "
        "instead of continuing to the next workflow/run."
    )
)
...
def restart_jaeger():
    """
    Restart the Jaeger Docker container before every test.
    """

    print("\n  Restarting Jaeger container...")
    print(f"  Container: {JAEGER_CONTAINER}")
    ...
    if result.returncode != 0:
        print("  -> FAILED to restart Jaeger.")
        print(f"  Docker error: {result.stderr.strip()}")
        return False

    print("  -> Jaeger restarted successfully.")

    # Give Jaeger a few seconds to become ready before starting
    # the actual load test.
    print("  Waiting 5s for Jaeger to initialize...")
    time.sleep(5)
...
print("=" * 60)
print(
    f"Running {len(WORKFLOWS)} workflows x "
    f"{args.runs} run(s) = {total_jobs} total jobs"
)
print(f"Sampling      : {args.sampling}")
print(f"Architecture  : {args.architecture}")
print(f"Jaeger        : {JAEGER_CONTAINER}")
print(f"Resume mode   : {args.resume}")
print("=" * 60)
...
for workflow in WORKFLOWS:
    ...
    for run in range(1, args.runs + 1):
        ...
        # --------------------------------------------------
        # Restart Jaeger BEFORE every new test
        # --------------------------------------------------

        if not restart_jaeger():

            print(
                f"  -> FAILED: Could not restart Jaeger "
                f"for {workflow['name']} run {run}"
            )
