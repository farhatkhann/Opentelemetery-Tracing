...
def route_display(route):
    """route is either 'METHOD /path' (monolith) or (service, 'METHOD /path') (microservices)."""
...
def main():
    ...
    for sampling in SAMPLINGS:
        ...
        union_ops = set()               # flat operationName union (monolith)
        union_service_pairs = set()     # (jaeger_service, operationName) union (microservices)
        total_traces = 0
        total_spans = 0
        per_workflow_ops = {}           # workflow -> set(flat op) or set((jaeger_service, op))
        ...
        for workflow, run, traces_path in entries:
            ...
            union_service_pairs |= set(service_op_counts.keys())
            total_traces += trace_count
            total_spans += span_count
            if is_microservices:
                per_workflow_ops.setdefault(workflow, set()).update(service_op_counts.keys())
            else:
        ...
        if is_microservices:
            ...
            for inv_service, route_name in routes:
                ...
                if jaeger_service and (jaeger_service, route_name) in union_service_pairs:
            ...
            per_workflow_route_hits = {
                wf: sorted(
                    f"[{inv_service}] {route_name}"
                    for inv_service, route_name in routes
                    if (svc_map.get(inv_service), route_name) in ops
                )
    ...
    write_excel(results, routes, excel_path)  # route_display() inside handles both tuple and flat routes
